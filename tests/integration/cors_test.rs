...
async fn test_cors_allowed_origin_returns_headers() {
    ...
    let resp = client
        ...
        .header("Origin", std::env::var("CORS_ALLOWED_ORIGINS").unwrap_or_else(|_| "http://localhost:8080".to_string()))
        .send()
        .await
        .expect("Failed to send request");

    let origin_header = resp.headers().get("access-control-allow-origin");
    assert!(origin_header.is_some(), "CORS header should be present for allowed origins");
}
...
async fn test_cors_disallowed_origin_no_headers() {
    ...
    if !std::env::var("CORS_ALLOWED_ORIGINS").unwrap_or_default().is_empty() {
